...
def login_student():
    ...
    if request.method == "POST":
        ...
        valid_password = verify_password((student or {}).get("password_hash", ""), raw_password)

        if not student or not valid_password:
            record_auth_failure("student_login", identity)
        ...
        if len(new_password) < 6:
            ...
            if not ok:
                ...
                cur.execute(
                    ...
                    (hash_password(new_password), admission_id)
                )
                db.commit()
                cur.close()
                db.close()
...
def student_dashboard():
    ...
    cur.execute("""
        SELECT
            ...
            spd.student_mobile,
            spd.caste_category,
            spd.alloted_category,
            spd.admission_quota,
            spd.residential_address,
            spd.qualifying_exam,
            spd.year_of_passing,
            spd.register_number,
            ed.total_max_marks,
        ...
        LEFT JOIN education_details ed ON ed.admission_id=s.admission_id
    ...
    cur.execute("""
        ...
        FROM fee_structure_master
        WHERE branch=%s AND semester_no=%s AND academic_year=%s
    """, ((student.get("branch") or ""), current_sem, ay))
    fee_structure = cur.fetchone() or {}
    ...
    paid_total = round(sum(float(p.get("amount") or 0) for p in payment_history), 2)
    due_total = round(
        float(fee_structure.get("admission_fee_due") or 0)
        + float(fee_structure.get("tuition_fee_due") or 0)
        + float(fee_structure.get("management_fee_due") or 0)
        + float(fee_structure.get("exam_fee_due") or 0),
        2
    )
...
def admission_letter():
    ...
    cur.execute("""
        SELECT
            ...
            COALESCE(sd.aadhaar_number, spd.aadhaar_number) AS aadhaar_number,
            COALESCE(sd.caste_rd_number, spd.caste_rd_number) AS caste_rd_number,
            COALESCE(sd.income_rd_number, spd.income_rd_number) AS income_rd_number,
            COALESCE(sd.marks_exam_type, spd.qualifying_exam) AS marks_exam_type,
            COALESCE(sd.student_photo, spd.photo_file, '') AS student_photo
        FROM students s
        LEFT JOIN education_details ed ON ed.admission_id=s.admission_id
        LEFT JOIN student_personal_details spd ON spd.admission_id=s.admission_id
    ...
    if not student:
        return "Student not found", 404
    if (student.get("status") or "").upper() != "ACTIVE":
        return "Admission letter is available only for approved admissions.", 403

    pdf = generate_admission_letter(
        student,
        photo_filename=student.get("student_photo")
    )
    return send_file(
        pdf,
        as_attachment=True,
        download_name=f"admission_{student.get('admission_id')}.pdf"
    )
...
def fee_receipt():
    ...
    if latest_payment:
        ...
        fees = {
            ...
            "payment_type": "NA",
            "payment_status": "NA",
            "receipt_no": student.get("admission_id"),
    ...
    return send_file(
        pdf,
        as_attachment=True,
        download_name=f"fee_receipt_{fees.get('receipt_no')}.pdf"
    )
